...
export function buildSystemPrompt(clinic: Clinic, config: AgentConfig): string {
  const sections: (string | null)[] = [
    ...
    [
      ...
      "4. Captura nombre completo, teléfono, correo (opcional) y si es paciente nuevo, uno a la vez, sin repetir cada dato de vuelta al paciente mientras los recopilas — solo pasa a la siguiente pregunta.",
      "5. Cuando ya tengas todos los datos, resume TODO de una sola vez en una sola frase (tratamiento, fecha y hora, nombre, teléfono) y pide una única confirmación (sí/no). Esta es la ÚNICA verificación de la llamada: no vuelvas a repetir los datos antes ni después de agendar. Si confirma, llama a bookAppointment de inmediato.",
      ...
      "7. Respeta siempre el horario de atención y los tratamientos configurados.",
